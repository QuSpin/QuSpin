...
sublat_list = [[(-1)**i/N,i] for i in range(0,L)]
# create static lists
operator_list_0 = [	
...
I = hamiltonian([["n|",sublat_list],["|n",sublat_list]],[],basis=basis,**no_checks)
...
psi_0[i_0] = 1.0 # set state to be in the given product state
...
def real(H_dict,I,psi_0,w,times,i):
	...
	times = U.grid # time grid stored in U
...
for w in w_list:	
	I_data = np.vstack([real(H_dict,I,psi_0,w,times,i) for i in range(n_real)])
	...
	# plotting results
	plt.errorbar(times,I_avg,I_error,marker=".",label="w={:.2f}".format(w))
##### configuring plots
