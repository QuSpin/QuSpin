...
def realization(H0, basis, psi_i, w, I, start, stop, num, endpoint, i):
    ...
    expH = exp_op(
        H0 + Hl, a=-1j, start=start, stop=stop, num=num, endpoint=endpoint, iterate=True
    )
    ...
    obs = obs_vs_time(
        psi_t, times, {"I": I}, basis=basis
    )  # ,Sent_args=dict(sparse=True,sub_sys_A=[0]))
...
nbs = 1000
n_data = [
    realization(H0, basis, psi_i, 5.0, I, start, stop, num, endpoint, i)
    for i in range(nbin)
]
...
nbs = 1000
n_data = [
    realization(H0, basis, psi_i, 1.0, I, start, stop, num, endpoint, i)
    for i in range(nbin)
]
# n_data,S_data = zip(*n_data)
