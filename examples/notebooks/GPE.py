...
for i, psi0 in enumerate(psi_tau):
    # compute energy
    E_GS = (
        Hsp.matrix_ele(psi0, psi0, time=0) + 0.5 * U * np.sum(np.abs(psi0) ** 4)
    ).real
    # plot wave function
    plt.plot(
        sites,
    ...
    plt.plot(sites, abs(psi0) ** 2, color="b", marker="o", label="$|\\phi_j(\\tau)|^2$")
    plt.xlabel("$\\mathrm{lattice\\ sites}$", fontsize=14)
    plt.title(
        "$J\\tau=%0.2f,\\ E_\\mathrm{GS}(\\tau)=%0.4fJ$" % (tau[i], E_GS), fontsize=14
    )
...
for i, psi in enumerate(psi_sp_t):
    ...
    plt.plot(
        ...
        color="b",
        marker="o",
        label="$|U(t,0)|\\psi_{\\mathrm{GS},j}\\rangle|^2$",
    )
    plt.plot(sites, kappa_trap, "--", color="g", label="$\\mathrm{trap}$")
