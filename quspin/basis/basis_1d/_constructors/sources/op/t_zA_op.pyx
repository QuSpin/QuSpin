...
cdef long double complex MatrixElement_ZA(int L,int zAblock, int kblock, int a, int l, long double k, int gA,NP_INT8_t Nr,NP_INT8_t Nc,NP_INT8_t mr,NP_INT8_t mc):
...
cdef int t_zA_op_template(op_type op_func,void *op_pars,shifter shift,bitop flip_sublat_A,void *ref_pars,int L,int kblock,int zAblock,int a, index_type Ns,
	...
	for i in range(Ns):
		s = row[i]
		RefState_T_ZA_template(shift,flip_sublat_A,s,L,a,&R[0],ref_pars)
		...
		if (matrix_type is float or matrix_type is double or matrix_type is longdouble):
			me = MatrixElement_ZA(L,zAblock,kblock,a,l,k,gA,N[i],N[ss],m[i],m[ss])
			ME[i] *= me.real
		else:
			ME[i] *= MatrixElement_ZA(L,zAblock,kblock,a,l,k,gA,N[i],N[ss],m[i],m[ss])
