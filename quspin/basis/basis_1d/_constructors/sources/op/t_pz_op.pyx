...
cdef long double MatrixElement_PZ(int L,int pzblock, int kblock, int a, int l, long double k, int qg,NP_INT8_t Nr,NP_INT8_t Nc,NP_INT8_t mr,NP_INT8_t mc):
...
cdef int t_pz_op_template(op_type op_func,void *op_pars,shifter shift,bitop fliplr,bitop flip_all,void *ref_pars,int L,int kblock,int pzblock,int a,
						index_type Ns, NP_INT8_t *N, NP_INT8_t *m, basis_type *basis, str opstr, NP_INT32_t *indx,
						scalar_type J, index_type *row, index_type *col, matrix_type *ME):
	cdef state_type s
	cdef index_type ss,i,j,o,p,c,b,l,qg 
	cdef int error
	cdef basis_type R[3]
	...
	if ((2*kblock*a) % L) == 0: #picks up k = 0, pi modes
		for i in range(Ns):
			s = row[i]
			RefState_T_PZ_template(shift,fliplr,flip_all,s,L,a,&R[0],ref_pars)
			...
			ME[i] *= MatrixElement_PZ(L,pzblock,kblock,a,l,k,qg,N[i],N[ss],m[i],m[ss])
		...
		for i in range(Ns):
			...
			if (ss == i) and (qg == 0) and (l == 0): #diagonal ME
				...
				for j in range(i,i+o,1):
					row[j] = j
					ME[j] *= MatrixElement_PZ(L,pzblock,kblock,a,l,k,qg,N[j],N[j],m[j],m[j])
				...
				for c in range(0,o,1):
					for b in range(0,p,1):
						j = i + c + Ns*b
						row[j] = ss + b
						ME[j] *= MatrixElement_PZ(L,pzblock,kblock,a,l,k,qg,N[i+c],N[ss+b],m[i+c],m[ss+b])
