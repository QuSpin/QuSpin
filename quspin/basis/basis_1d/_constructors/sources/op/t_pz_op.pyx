...
def f_t_pz_op(_np.ndarray[NP_INT8_t,ndim=1] N,_np.ndarray[NP_INT8_t,ndim=1] m, _np.ndarray[NP_UINT32_t,ndim=1] basis, str opstr, _np.ndarray[NP_INT32_t,ndim=1] indx, float complex J, int L, int pzblock, int kblock, int a):
	cdef unsigned int s
...
def d_t_pz_op(_np.ndarray[NP_INT8_t,ndim=1] N,_np.ndarray[NP_INT8_t,ndim=1] m, _np.ndarray[NP_UINT32_t,ndim=1] basis, str opstr, _np.ndarray[NP_INT32_t,ndim=1] indx, double complex J, int L, int pzblock, int kblock, int a):
	cdef unsigned int s
	cdef int error,ss,i,j,Ns,o,p,c,b,l,qg
	cdef _np.ndarray[NP_UINT32_t,ndim = 1] R = _np.zeros(3,NP_UINT32)
	cdef _np.ndarray[NP_INT32_t,ndim=1] row
...
cdef long double g_MatrixElement_PZ(int L,int pzblock, int kblock, int a, int l, long double k, int qg,NP_INT8_t Nr,NP_INT8_t Nc,NP_INT8_t mr,NP_INT8_t mc):
...
def g_t_pz_op(_np.ndarray[NP_INT8_t,ndim=1] N,_np.ndarray[NP_INT8_t,ndim=1] m, _np.ndarray[NP_UINT32_t,ndim=1] basis, str opstr, _np.ndarray[NP_INT32_t,ndim=1] indx, long double complex J, int L, int pzblock, int kblock, int a):
	cdef unsigned int s
	cdef int error,ss,i,j,Ns,o,p,c,b,l,qg
	cdef _np.ndarray[NP_UINT32_t,ndim = 1] R = _np.zeros(3,NP_UINT32)
	cdef _np.ndarray[NP_INT32_t,ndim=1] row
	...
	if ((2*kblock*a) % L) == 0: #picks up k = 0, pi modes
		for i in range(Ns):
			...
			ME[i] *= g_MatrixElement_PZ(L,pzblock,kblock,a,l,k,qg,N[i],N[ss],m[i],m[ss])
		...
		for i in range(Ns):
			...
			if (ss == i) and (qg == 0) and (l == 0): #diagonal ME
				...
				for j in range(i,i+o,1):
					row[j] = j
					ME[j] *= g_MatrixElement_PZ(L,pzblock,kblock,a,l,k,qg,N[j],N[j],m[j],m[j])
				...
				for c in range(0,o,1):
					for b in range(0,p,1):
						j = i + c + Ns*b
						row[j] = ss + b
						ME[j] *= g_MatrixElement_PZ(L,pzblock,kblock,a,l,k,qg,N[i+c],N[ss+b],m[i+c],m[ss+b])
