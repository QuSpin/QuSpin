...
def f_t_p_z_op(_np.ndarray[NP_INT8_t,ndim=1] N,_np.ndarray[NP_INT16_t,ndim=1] m, _np.ndarray[basis_type,ndim=1] basis, str opstr, _np.ndarray[NP_INT32_t,ndim=1] indx, float complex J, int L, int pblock,int zblock, int kblock, int a):
	cdef state_type s
	...
	if ((2*kblock*a) % L) == 0: #picks up k = 0, pi modes
		for i in range(Ns):
			s = row[i]
			RefState_T_P_Z_template(shift,fliplr,flip_all,s,L,a,&R[0])
...
def d_t_p_z_op(_np.ndarray[NP_INT8_t,ndim=1] N,_np.ndarray[NP_INT16_t,ndim=1] m, _np.ndarray[basis_type,ndim=1] basis, str opstr, _np.ndarray[NP_INT32_t,ndim=1] indx, double complex J, int L, int pblock,int zblock, int kblock, int a):
	cdef state_type s
	cdef int error,ss,i,j,Ns,o,p,c,b,l,q,g
	cdef _np.ndarray[basis_type,ndim=1] R = _np.zeros(4,dtype=basis.dtype)
	cdef _np.ndarray[NP_INT32_t,ndim=1] row
