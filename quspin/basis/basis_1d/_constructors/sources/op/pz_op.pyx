...
cdef int pz_op_template(op_type op_func,void *op_pars,bitop fliplr,bitop flip_all,void *ref_pars,int L,int pzblock, index_type Ns,
	...
	for i in range(Ns):
		s = row[i]
		RefState_PZ_template(fliplr,flip_all,s,L,&R[0],ref_pars)
