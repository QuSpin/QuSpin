...
# parity 
def fermion_n_p_basis(int L,int Nup,int pblock,_np.ndarray[N_type,ndim=1] N,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
...
def fermion_p_basis(int L,int pblock,_np.ndarray[N_type,ndim=1] N, _np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s = 0
...
def fermion_n_p_z_basis(int L, int Nup, int pblock, int zblock, _np.ndarray[N_type,ndim=1] N, _np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def fermion_p_z_basis(int L, int pblock, int zblock, _np.ndarray[N_type,ndim=1] N, _np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
# (parity)*(spin inversion)
def fermion_n_pz_basis(int L, int Nup, int pzblock, _np.ndarray[N_type,ndim=1] N, _np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def fermion_pz_basis(int L, int pzblock, _np.ndarray[N_type,ndim=1] N, _np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
# translation
def fermion_n_t_basis(int L, int Nup, int kblock,int a, _np.ndarray[N_type,ndim=1] N, _np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def fermion_t_basis(int L, int kblock,int a, _np.ndarray[N_type,ndim=1] N, _np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
# translation-parity
def fermion_n_t_p_basis(int L, int Nup,int pblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def fermion_t_p_basis(int L,int pblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
def fermion_n_t_p_z_basis(int L, int Nup,int pblock,int zblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[m_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def fermion_t_p_z_basis(int L,int pblock,int zblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[m_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
# translation-(parity)*(spin inversion)
def fermion_n_t_pz_basis(int L, int Nup,int pzblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def fermion_t_pz_basis(int L,int pzblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
def fermion_n_t_z_basis(int L,int Nup,int zblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def fermion_t_z_basis(int L,int zblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
def fermion_n_t_zA_basis(int L, int Nup,int zAblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def fermion_t_zA_basis(int L,int zAblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
def fermion_n_t_zB_basis(int L, int Nup,int zBblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def fermion_t_zB_basis(int L,int zBblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
def fermion_n_t_zA_zB_basis(int L,int Nup,int zAblock,int zBblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[m_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def fermion_t_zA_zB_basis(int L,int zAblock,int zBblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[m_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
