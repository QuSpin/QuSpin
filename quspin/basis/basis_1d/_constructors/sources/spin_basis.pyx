...
# magnetization 
def spin_n_basis(int L, int Nup, npy_uintp Ns,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef int j
...
# parity 
def spin_n_p_basis(int L,int Nup,int pblock,_np.ndarray[N_type,ndim=1] N,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
...
def spin_p_basis(int L,int pblock,_np.ndarray[N_type,ndim=1] N, _np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s = 0
...
def spin_n_p_z_basis(int L, int Nup, int pblock, int zblock, _np.ndarray[N_type,ndim=1] N, _np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def spin_p_z_basis(int L, int pblock, int zblock, _np.ndarray[N_type,ndim=1] N, _np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
# (parity)*(spin inversion)
def spin_n_pz_basis(int L, int Nup, int pzblock, _np.ndarray[N_type,ndim=1] N, _np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def spin_pz_basis(int L, int pzblock, _np.ndarray[N_type,ndim=1] N, _np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
# translation
def spin_n_t_basis(int L, int Nup, int kblock,int a, _np.ndarray[N_type,ndim=1] N, _np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def spin_t_basis(int L, int kblock,int a, _np.ndarray[N_type,ndim=1] N, _np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
# translation-parity
def spin_n_t_p_basis(int L, int Nup,int pblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def spin_t_p_basis(int L,int pblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
def spin_n_t_p_z_basis(int L, int Nup,int pblock,int zblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[m_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def spin_t_p_z_basis(int L,int pblock,int zblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[m_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
# translation-(parity)*(spin inversion)
def spin_n_t_pz_basis(int L, int Nup,int pzblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def spin_t_pz_basis(int L,int pzblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
def spin_n_t_z_basis(int L,int Nup,int zblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def spin_t_z_basis(int L,int zblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
def spin_n_t_zA_basis(int L, int Nup,int zAblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def spin_t_zA_basis(int L,int zAblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
def spin_n_t_zB_basis(int L, int Nup,int zBblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def spin_t_zB_basis(int L,int zBblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[N_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
...
def spin_n_t_zA_zB_basis(int L,int Nup,int zAblock,int zBblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[m_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s
    cdef npy_uintp MAX=comb(L,Nup,exact=True)
    cdef int j
    s = 0
...
def spin_t_zA_zB_basis(int L,int zAblock,int zBblock,int kblock,int a,_np.ndarray[N_type,ndim=1] N,_np.ndarray[m_type,ndim=1] m,_np.ndarray[basis_type,ndim=1] basis):
    cdef npy_uintp s=0
    cdef npy_uintp MAX=1ull<<L
    return make_t_zA_zB_basis_template[basis_type,N_type,m_type](shift,flip_sublat_A,flip_sublat_B,flip_all,next_state_inc_1,None,MAX,s,L,zAblock,zBblock,kblock,a,&N[0],&m[0],basis)
