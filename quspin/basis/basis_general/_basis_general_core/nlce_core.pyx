...
# specialized code 
cdef extern from "nlce_site_basis_core.h" namespace "nlce::nlce_site":
...
ctypedef pair[int,int] Edge_t
ctypedef set[Edge_t] EdgeSet_t
ctypedef unordered_map[int,map[int,int]] EdgeWeights_t
ctypedef map[int,EdgeSet_t] EdgeDict_t
ctypedef unordered_map[int,EdgeDict_t] Edges_t

cdef extern from "nlce_plaquet_basis_core.h" namespace "nlce::nlce_plaquet":
    cdef cppclass nlce_plaquet_basis_core[I]:
        nlce_plaquet_basis_core(const int,const int,const int[],const int,
            Edges_t,const int,const int,const int[],const int[],const int[])
        nlce_plaquet_basis_core(const int,const int,const int[],const int,
            Edges_t,EdgeWeights_t,const int,const int,const int[],const int[],const int[])
        void clusters_calc()
        void calc_subclusters()
        void get_Y_matrix_dims(int&,int&)
        void get_Y_matrix(npy_intp[],npy_intp[],npy_intp[])
        void cluster_copy[J,K](npy_intp,J[],K[],npy_intp[])



cdef class nlce_plaquet_core_wrap:
    cdef int _Ncl
    cdef int _N
    cdef void * _nlce_core
    ...
    def __cinit__(self,int Ncl
                      ...
                      ,int[::1] qs
                      ,int[:,::1] plaquet_sites
                      ,object plaquet_edges
                      ,object edge_weights=None):
        
        cdef int N = maps.shape[1]
        cdef int Nsp = plaquet_sites.shape[1]
        cdef Edges_t plaquet_edges_c
        cdef EdgeWeights_t edge_weights_c
        cdef int a,b,i,j
        ...
        for a,edge_dict in plaquet_edges.items():
            for b,edge_set in edge_dict.items():
                plaquet_edges_c[a][b] = set[pair[int,int]]()
                
                for (i,j) in edge_set:
                    plaquet_edges_c[a][b].insert(pair[int,int](i,j))

        if edge_weights is None:
            if N <= 64:
                self._nlce_core = <void*> new nlce_plaquet_basis_core[uint64_t](Ncl,N,&plaquet_sites[0,0],Nsp,plaquet_edges_c,nt_point,nt_tran,&maps[0,0],&pers[0],&qs[0])        
            elif N <= 128:
                self._nlce_core = <void*> new nlce_plaquet_basis_core[uint128_t](Ncl,N,&plaquet_sites[0,0],Nsp,plaquet_edges_c,nt_point,nt_tran,&maps[0,0],&pers[0],&qs[0])
            elif N <= 256:
                self._nlce_core = <void*> new nlce_plaquet_basis_core[uint256_t](Ncl,N,&plaquet_sites[0,0],Nsp,plaquet_edges_c,nt_point,nt_tran,&maps[0,0],&pers[0],&qs[0])
            elif N <= 512:
                self._nlce_core = <void*> new nlce_plaquet_basis_core[uint512_t](Ncl,N,&plaquet_sites[0,0],Nsp,plaquet_edges_c,nt_point,nt_tran,&maps[0,0],&pers[0],&qs[0])
            elif N <= 1024:
                self._nlce_core = <void*> new nlce_plaquet_basis_core[uint1024_t](Ncl,N,&plaquet_sites[0,0],Nsp,plaquet_edges_c,nt_point,nt_tran,&maps[0,0],&pers[0],&qs[0])
            else:
                raise ValueError
        else:
            for (i,j),W in edge_weights.items():
                edge_weights_c[i][j] = <int>W;

            if N <= 64:
                self._nlce_core = <void*> new nlce_plaquet_basis_core[uint64_t](Ncl,N,&plaquet_sites[0,0],Nsp,plaquet_edges_c,edge_weights_c,nt_point,nt_tran,&maps[0,0],&pers[0],&qs[0])        
            elif N <= 128:
                self._nlce_core = <void*> new nlce_plaquet_basis_core[uint128_t](Ncl,N,&plaquet_sites[0,0],Nsp,plaquet_edges_c,edge_weights_c,nt_point,nt_tran,&maps[0,0],&pers[0],&qs[0])
            elif N <= 256:
                self._nlce_core = <void*> new nlce_plaquet_basis_core[uint256_t](Ncl,N,&plaquet_sites[0,0],Nsp,plaquet_edges_c,edge_weights_c,nt_point,nt_tran,&maps[0,0],&pers[0],&qs[0])
            elif N <= 512:
                self._nlce_core = <void*> new nlce_plaquet_basis_core[uint512_t](Ncl,N,&plaquet_sites[0,0],Nsp,plaquet_edges_c,edge_weights_c,nt_point,nt_tran,&maps[0,0],&pers[0],&qs[0])
            elif N <= 1024:
                self._nlce_core = <void*> new nlce_plaquet_basis_core[uint1024_t](Ncl,N,&plaquet_sites[0,0],Nsp,plaquet_edges_c,edge_weights_c,nt_point,nt_tran,&maps[0,0],&pers[0],&qs[0])
            else:
                raise ValueError

    def _calc_topo_clusters(self):
        cdef void * nlce_core = self._nlce_core
        cdef nlce_plaquet_basis_core[uint64_t]* nlce_64 = <nlce_plaquet_basis_core[uint64_t]*>nlce_core
        cdef nlce_plaquet_basis_core[uint128_t]* nlce_128 = <nlce_plaquet_basis_core[uint128_t]*>nlce_core
        cdef nlce_plaquet_basis_core[uint256_t]* nlce_256 = <nlce_plaquet_basis_core[uint256_t]*>nlce_core
        cdef nlce_plaquet_basis_core[uint512_t]* nlce_512 = <nlce_plaquet_basis_core[uint512_t]*>nlce_core
        cdef nlce_plaquet_basis_core[uint1024_t]* nlce_1024 = <nlce_plaquet_basis_core[uint1024_t]*>nlce_core       
    ...
    def _get_cluster_data(self,_npc.ndarray clusters,_npc.ndarray ncl,_npc.ndarray L):
        ...
        cdef int8_t * ncl_ptr = <int8_t*>_npc.PyArray_DATA(ncl)
        cdef npy_intp * L_ptr = <npy_intp*>_npc.PyArray_DATA(L)
        cdef npy_intp c_row = clusters.shape[1]

        if self._N <= 64:
            nlce_64.cluster_copy(c_row,clusters_ptr,ncl_ptr,L_ptr)
        elif self._N <= 128:
            nlce_128.cluster_copy(c_row,clusters_ptr,ncl_ptr,L_ptr)
        elif self._N <= 256:
            nlce_256.cluster_copy(c_row,clusters_ptr,ncl_ptr,L_ptr)
        elif self._N <= 512:
            nlce_512.cluster_copy(c_row,clusters_ptr,ncl_ptr,L_ptr)
        elif self._N <= 1024:
            nlce_1024.cluster_copy(c_row,clusters_ptr,ncl_ptr,L_ptr)
    ...
    def calc_clusters(self):
        ...
        L        = _np.zeros(row          ,dtype=_np.intp)
        ncl      = _np.zeros(row          ,dtype=_np.int8)
        clusters = _np.zeros((row,self._Ncl-1),dtype=_np.int16)
        data     = _np.zeros(nnz          ,dtype=_np.intp)
        indptr   = _np.zeros(row+1        ,dtype=_np.intp)
        indices  = _np.zeros(nnz          ,dtype=_np.intp)
