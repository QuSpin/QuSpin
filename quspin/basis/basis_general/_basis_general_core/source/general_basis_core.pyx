...
cdef class general_basis_core_wrap:
    ...
    # @cython.boundscheck(False)
    # def inplace_op(self,dtype[:,::1] v_in,type[:,::1] v_out,object opstr,int[::1] indx,object J,_np.ndarray basis,norm_type[::1] n):
    #     cdef char[::1] c_opstr = bytearray(opstr,"utf-8")
    #     cdef int n_op = indx.shape[0]
    #     cdef npy_intp Ns = basis.shape[0]
    ...
    #     if basis.dtype == _np.uint32:
    #         with nogil:
    #             err = general_inplace_op(<general_basis_core[uint32_t]*>B,n_op,&c_opstr[0],&indx[0],JJ,Ns,nvecs,<uint32_t*>basis_ptr,&n[0],&v_in[0,0],&v_out[0,0])
    #     elif basis.dtype == _np.uint64:
    #         with nogil:
    #             err = general_inplace_op(<general_basis_core[uint64_t]*>B,n_op,&c_opstr[0],&indx[0],JJ,Ns,nvecs,<uint64_t*>basis_ptr,&n[0],&v_in[0,0],&v_out[0,0])
    #     else:
    #         raise TypeError("basis dtype must be either uint32 or uint64")
