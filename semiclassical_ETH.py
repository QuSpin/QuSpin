...
L = 2		
Nph_tot = 2 #120 #total number of photon states
...
Pstatic = [["zz|",J_nn], ["z|",z_field], ["x|",x_field]]
...
Pbasis = photon_basis(spin_basis_1d,L=L,Nph=Nph_tot,kblock=0,pblock=1)
...
PHsp = hamiltonian(Pstatic,[],L=L,dtype=np.float64,basis=Pbasis)
...
print "total and spin H-space sizes are", PH.Ns, H.Ns
...
PEnergy = np.real( np.einsum("ij,jk,ik->i", Ppsi.conj(),PHsp.todense(),Ppsi) )
...
plt.plot(t/T,PEnergy/L,'b',linewidth=1,label=str1)
plt.plot(t/T,Energy/L,'r--',linewidth=2,label=str2)
