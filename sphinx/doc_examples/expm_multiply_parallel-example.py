...
from quspin.tools import expm_multiply_parallel # expm_multiply_parallel
...
work_array=np.zeros((2*len(psi),), dtype=np.float64)
print(H.expt_value(psi)) # measure energy of state |psi>
expH.dot(psi,work=work_array,overwrite_v=True) # compute action of matrix exponential on a state
