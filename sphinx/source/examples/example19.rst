...
which is the expression we use in the code snippet below. 
...
.. math::
	...
	\times 
	\left(\frac{1}{m_Q}\sum_{j'=0}^{m_Q-1}\mathrm{e}^{+i \frac{2\pi q}{m_Q} j' } (Q^{j'})^\dagger \mathcal{O}_{0}(0) Q^{j'}  \right)
