...
def test():
    ...
    for state, state_sp in zip([V[:, 0], V], [sp.csr_matrix(V[:, 0]).T, sp.csr_matrix(V)]):
        ...
        np.testing.assert_allclose(
            sent["rdm_A"] - rdm_A_sp, 0.0, atol=1e-5, err_msg="Failed rdm_A comparison!"
        )
        ...
        np.testing.assert_allclose(
            sent["Sent_B"] - sent_sp["Sent_B"],
            0.0,
            atol=1e-5,
            err_msg="Failed Sent comparison!",
        )
        np.testing.assert_allclose(
            sent["rdm_B"] - rdm_B_sp, 0.0, atol=1e-5, err_msg="Failed rdm_A comparison!"
        )

        #####

        sent = basis.ent_entropy(state, sub_sys_A, return_rdm="both", enforce_pure=True)
        sent_sp = basis.ent_entropy(
            state_sp, sub_sys_A, return_rdm="both", enforce_pure=True
        )
