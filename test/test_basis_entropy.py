...
def test():
    ...
    np.testing.assert_allclose(
        sent["Sent"] - sent_b["Sent_A"], 0.0, atol=1e-5, err_msg="Failed Sent comparison!"
    )
    ...
    sent = _ent_entropy(
        state, basis, chain_subsys=sub_sys_A, DM="chain_subsys", density=False
    )
    ...
    np.testing.assert_allclose(
        sent["DM_chain_subsys"] - sent_b["rdm_A"],
        0.0,
        atol=1e-5,
        err_msg="Failed rdm_A comparison!",
    )
    ...
    sent = _ent_entropy(
        state, basis, chain_subsys=sub_sys_A, DM="other_subsys", density=False
    )
    ...
    np.testing.assert_allclose(
        sent["DM_other_subsys"] - sent_b["rdm_B"],
        0.0,
        atol=1e-5,
        err_msg="Failed rdm_B comparison!",
    )
    ...
    for state, enforce_pure in zip([V, V[:, 1:6]], [True, False]):
        ...
        #####
        sent_b = basis.ent_entropy(
            state, sub_sys_A, enforce_pure=enforce_pure, density=False
        )

        sent = _ent_entropy(
            {"V_states": state}, basis, chain_subsys=sub_sys_A, density=False
        )

        np.testing.assert_allclose(
            sent["Sent"] - sent_b["Sent_A"],
            0.0,
            atol=1e-5,
            err_msg="Failed Sent comparison!",
        )

        #####

        sent_b = basis.ent_entropy(
            state, sub_sys_A, return_rdm="A", enforce_pure=enforce_pure, density=False
        )
        sent = _ent_entropy(
            {"V_states": state}, basis, sub_sys_A, DM="chain_subsys", density=False
        )
        ...
        sent_b = basis.ent_entropy(
            state, sub_sys_A, return_rdm="B", enforce_pure=enforce_pure, density=False
        )
        sent = _ent_entropy(
            {"V_states": state},
            basis,
        ...
        sent_b = basis.ent_entropy(
            state, sub_sys_A, return_rdm="both", enforce_pure=enforce_pure, density=False
        )
        sent = _ent_entropy(
            {"V_states": state}, basis, chain_subsys=sub_sys_A, DM="both", density=False
        )
    ...
    for DM in [DM1, DM2, DM3]:
        ...
        sent = _ent_entropy(
            DM.squeeze(), basis, chain_subsys=sub_sys_A, DM="both", density=False
        )
        ...
        sent = _ent_entropy(
            DM.squeeze(), basis, chain_subsys=sub_sys_A, DM="chain_subsys", density=False
        )
        ...
        sent = _ent_entropy(
            DM.squeeze(), basis, chain_subsys=sub_sys_A, DM="other_subsys", density=False
        )

        np.testing.assert_allclose(
            sent["Sent"] - sent_b["Sent_B"],
            0.0,
            atol=1e-5,
            err_msg="Failed Sent comparison!",
        )
    ...
    np.testing.assert_allclose(
        np.diff(sent_b["Sent_A"]), 0.0, atol=1e-5, err_msg="Failed Sent comparison!"
    )
    #####
    ...
    np.testing.assert_allclose(
        np.diff(sent_b["rdm_A"], axis=0), 0.0, atol=1e-5, err_msg="Failed rdm_A comparison!"
    )
    np.testing.assert_allclose(
        np.diff(sent_b["rdm_B"], axis=0), 0.0, atol=1e-5, err_msg="Failed rdm_B comparison!"
    )
    ...
    np.testing.assert_allclose(
        np.diff(sent_b["rdm_A"], axis=0), 0.0, atol=1e-5, err_msg="Failed rdm_B comparison!"
    )
    ...
    np.testing.assert_allclose(
        np.diff(sent_b["Sent_B"]), 0.0, atol=1e-5, err_msg="Failed Sent comparison!"
    )
    np.testing.assert_allclose(
        np.diff(sent_b["rdm_B"], axis=0), 0.0, atol=1e-5, err_msg="Failed rdm_B comparison!"
    )
