...
def check_zA(L, dtype):
    ...
    if norm(Ez - E) > Ns * eps(dtype):
        raise Exception(
            "test failed zA symmetry at L={0:3d} with dtype {1} and {2}".format(
                L, np.dtype(dtype), norm(Ez - E)
            )
        )
...
def check_zB(L, dtype):
    ...
    if norm(Ez - E) > Ns * eps(dtype):
        raise Exception(
            "test failed zB symmetry at L={0:3d} with dtype {1} and {2}".format(
                L, np.dtype(dtype), norm(Ez - E)
            )
        )
...
def check_zA_zB(L, dtype):
    ...
    if norm(Ez - E) > Ns * eps(dtype):
        raise Exception(
            "test failed zA zB symmetry at L={0:3d} with dtype {1} and {2}".format(
                L, np.dtype(dtype), norm(Ez - E)
            )
        )
...
def check_t_z(L, dtype, Nup=None):
    ...
    for kblock in range(-L_2 + 1, L_2 + 1):
        ...
        Hk1 = hamiltonian(
            static, [], N=L, Nup=Nup, dtype=dtype, kblock=kblock, zblock=+1
        )
        Hk2 = hamiltonian(
            static, [], N=L, Nup=Nup, dtype=dtype, kblock=kblock, zblock=-1
        )
...
def check_t_zA(L, dtype, a=2):
    ...
    for kblock in range(-L_2 + 2, L_2 + 2):
        ...
        if norm(Ek - Ekz) > Ns * eps(dtype):
            raise Exception(
                "test failed t zA symmetry at L={0:3d} with dtype {1} and {2}".format(
...
def check_t_zB(L, dtype, a=2):
    ...
    for kblock in range(-L_2 + 2, L_2 + 2):
        ...
        if norm(Ek - Ekz) > Ns * eps(dtype):
            raise Exception(
                "test failed t zB symmetry at L={0:3d} with dtype {1} and {2}".format(
                    L, np.dtype(dtype), norm(Ek - Ekz)
                )
            )
...
def check_t_zA_zB(L, dtype, a=2):
    ...
    for kblock in range(0, L_2):
        ...
        Hk1 = hamiltonian(
            static, [], N=L, dtype=dtype, kblock=kblock, zAblock=+1, zBblock=+1, a=a
        )
        Hk2 = hamiltonian(
            static, [], N=L, dtype=dtype, kblock=kblock, zAblock=+1, zBblock=-1, a=a
        )
...
def check_t_p(L, dtype, Nup=None):
    ...
    for kblock in range(-L_2 + 1, 0):
        ...
        Hk1 = hamiltonian(
            static, [], N=L, Nup=Nup, dtype=dtype, kblock=kblock, pblock=+1
        )
        ...
        if norm(Ek - Ek1) > Ns * eps(dtype):
            raise Exception(
                "test failed t p+ symmetry at L={0:3d} kblock={1:3d} with dtype {2} and Nup={3} {4}".format(
...
def check_t_pz(L, dtype, Nup=None):
    ...
    for kblock in range(-L_2 + 1, 0):
        ...
        Hk1 = hamiltonian(
            static, [], N=L, Nup=Nup, dtype=dtype, kblock=kblock, pzblock=+1, a=a
        )
        Hk2 = hamiltonian(
            static, [], N=L, Nup=Nup, dtype=dtype, kblock=kblock, pzblock=-1, a=a
        )
    ...
    if (L / a) % 2 == 0:
        ...
        Hk1 = hamiltonian(
            static, [], N=L, Nup=Nup, dtype=dtype, kblock=L_2, pzblock=+1, a=a
        )
        Hk2 = hamiltonian(
            static, [], N=L, Nup=Nup, dtype=dtype, kblock=L_2, pzblock=-1, a=a
        )
...
def check_t_p_z(L, dtype, Nup=None):
    ...
    for kblock in range(-L_2 + 1, L_2 + 1):
        ...
        Hkpz11 = hamiltonian(
            static, [], N=L, Nup=Nup, dtype=dtype, kblock=kblock, pblock=+1, zblock=+1
        )
        Hkpz12 = hamiltonian(
            static, [], N=L, Nup=Nup, dtype=dtype, kblock=kblock, pblock=+1, zblock=-1
        )
