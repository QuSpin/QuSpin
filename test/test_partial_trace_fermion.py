...
def test_pure_states(N=3, sparse=False, enforce_pure=False):
    ...
    rho_red = basis_full.partial_trace(
        psi_full,
        sub_sys_A=sub_sys_A,
        return_rdm="A",
    ...
    np.testing.assert_allclose(
        A_full.expt_value(psi_full) - A_red.expt_value(rho_red.T, enforce_pure=False),
        0.0,
        atol=1e-5,
...
def test_mixed_states(N=3, sparse=False, enforce_pure=False):
    ...
    if sparse:
        ...
        if N > 1:
            rdm_full = np.zeros(
                (basis_full.Ns, basis_full.Ns, N),
            )
        else:
            rdm_full = np.zeros(
                (basis_full.Ns, basis_full.Ns),
            )
        ...
        if N > 1:
            rdm_full = np.random.uniform(
                -1.0, 1.0, size=(basis_full.Ns, basis_full.Ns, N)
            )
    ...
    np.testing.assert_allclose(
        A_full.expt_value(rdm_full) - A_red.expt_value(rho_red.T, enforce_pure=False),
        0.0,
        atol=1e-5,
        err_msg="failed local operator comparison!",
    )
...
for spinful, basis in enumerate(spinless_bases_list + spinful_bases_list):
    ...
    for sub_sys_A in combinations(range(basis_full.N), 2):
        ...
        if spinful:
            ...
            if np.min(subsysA) <= L - 1 and np.max(subsysA) > L - 1:
                basis_red = spinful_fermion_basis_1d(
                    1,
                )
                ...
                basis_red = spinless_fermion_basis_1d(
                    2,
                )
                ...
                basis_red = spinless_fermion_basis_general(
                    2,
                )
        ...
        A_full = hamiltonian(
            static_full, [], basis=basis_full, dtype=np.float64, **no_checks
        )
        A_red = hamiltonian(
            static_red, [], basis=basis_red, dtype=np.float64, **no_checks
        )
