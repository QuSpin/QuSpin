...
def spin_entropy(L, dtype, symm, Sent_args):
...
for _r in range(10):  # do 10 random checks
    ...
    for dtype_str in dtypes.keys():
        ...
        for symm in [0, 1]:
            ...
            DM_other_subsys = []
            S2 = []
            for _i, _s in enumerate(spin_entropy(L, dtype, symm, Sent_args)):
            ...
            spin_entropy(L, dtype, symm, Sent_args)
    ...
    for _r in range(10):  # do 10 random checks
        ...
        for dtype_str in dtypes.keys():
            ...
            for symm in [0, 1]:
                ...
                np.testing.assert_allclose(np.diff(S1),0.0,atol=atol,err_msg='Failed entropies comparison!')
                if symm == 1:
                        np.testing.assert_allclose(np.diff(S.ravel()),0.0,atol=atol,
