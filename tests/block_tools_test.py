...
def test():
    ...
    H_block = block_diag_hamiltonian(
        blocks, static, dynamic, spin_basis_1d, (L,), np.float64
    )
    ...
    # real time.
    expH = exp_op(
        H, a=-1j, start=start, stop=stop, iterate=True, num=num, endpoint=True
    )
    ...
    psi_block_1 = block_op.evolve(
        psi0, 0, times, iterate=True, atol=1e-15, rtol=1e-15, n_jobs=4
    )

    psi_exact_2 = expH.dot(psi0, time=0.3)
    psi_block_2 = block_op.expm(
        psi0,
        H_time_eval=0.3,
        start=start,
        stop=stop,
    ...
    for psi_e_1, psi_e_2, psi_b_1, psi_b_2 in izip(
        psi_exact_1, psi_exact_2, psi_block_1, psi_block_2
    ):
    ...
    psi_block_1 = block_op.evolve(
        psi0, 0, times, iterate=False, atol=1e-15, rtol=1e-15, n_jobs=4
    )
