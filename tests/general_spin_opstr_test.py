        ...
        for l in range(1, l_max + 1):
            for i0 in range(0, L - l + 1, 1):
                ...
                for opstr in product(*[ops for i in range(l)]):
                    ...
                    err_msg = "testing: {opstr:} {indx:}  S={S:} Nup={Nup:} kblock={kblock:} pblock={pblock:} zblock={zblock:}".format(
                        **printing
                    )
...
def test_gen_basis_spin_boost(L, Nups, l_max, S="1/2"):
    ...
    for Nup, kblock, pblock in product(Nups, kblocks, pblocks):
        ...
        print(
            "checking S={S:} Nup={Nup:} kblock={kblock:} pblock={pblock:}".format(
                Nup=Nup, **basis_blocks
            )
        )
        if basis_1d.Ns != gen_basis.Ns:
        ...
        for l in range(1, l_max + 1):
            for i0 in range(0, L - l + 1, 1):
                ...
                for opstr in product(*[ops for i in range(l)]):
                    ...
                    err_msg = "testing: {opstr:} {indx:}  S={S:} Nup={Nup:} kblock={kblock:} pblock={pblock:}".format(
                        **printing
                    )
