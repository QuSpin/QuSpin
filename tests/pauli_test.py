...
def compare_inplace_Op(static_list, basis_1, basis_2, ratio_12, test_ratio=True):
    for opstr, indx, J in static_list:
        ...
        v_out_1 = basis_1.inplace_Op(
            v_in, op_list, dtype, transposed=False, conjugated=False, v_out=None
        )
        v_out_2 = basis_2.inplace_Op(
            v_in, op_list, dtype, transposed=False, conjugated=False, v_out=None
        )
...
bases = [
    (basis_1d_S, basis_1d_pauli_1, basis_1d_pauli_2),
    (spin_basis_general_S, spin_basis_general_pauli_1, spin_basis_general_pauli_2),
]
...
for basis_S, basis_pauli_1, basis_pauli_2 in bases:
    ...
    for static in opstr_lists:
        ...
        compare_inplace_Op(
            static_list, basis_S, basis_pauli_1, 1 / 2**2, test_ratio=False
        )
        compare_inplace_Op(
            static_list, basis_S, basis_pauli_2, 1 / 2**2, test_ratio=True
        )
        ...
        if isinstance(basis_S, spin_basis_general):
            ...
            compare_Op_bra_ket(
                static_list, basis_S, basis_pauli_1, 0.25, test_ratio=False
            )
            compare_Op_bra_ket(
                static_list, basis_S, basis_pauli_2, 0.25, test_ratio=True
            )
