...
psi = np.random.uniform(-1, 1, size=(basis.Ns, 1)) + 1j * np.random.uniform(
    -1, 1, size=(basis.Ns, 1)
)
psi /= np.linalg.norm(psi, axis=0)
psis = np.random.uniform(-1, 1, size=(basis.Ns, 100)) + 1j * np.random.uniform(
    -1, 1, size=(basis.Ns, 100)
)
...
n_state = 10
psis = np.exp(
    -1j * np.random.uniform(-np.pi, np.pi, size=(basis.Ns, n_state))
) / np.sqrt(basis.Ns)
