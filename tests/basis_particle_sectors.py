...
for L in [6,7]:
	...
	basis_4=spinful_fermion_basis_1d(L=L,Nf=(range(0,L,2),range(0,L,2)) )
	basis_4g=spinful_fermion_basis_general(N=L,Nf=(range(0,L,2),range(0,L,2)) )


	# symmetry-ful

	basis_1=spin_basis_1d(L=L,Nup=range(0,L,2),kblock=0)
	basis_1g=spin_basis_general(N=L,Nup=range(0,L,2),kblock=0)

	basis_2=boson_basis_1d(L=L,Nb=range(0,L,2),kblock=0)
	basis_2g=boson_basis_general(N=L,Nb=range(0,L,2),kblock=0)

	basis_3=spinless_fermion_basis_1d(L=L,Nf=range(0,L,2),kblock=0)
	basis_3g=spinless_fermion_basis_general(N=L,Nf=range(0,L,2),kblock=0)

	basis_4=spinful_fermion_basis_1d(L=L,Nf=(range(0,L,2),range(0,L,2)),kblock=0 )
	basis_4g=spinful_fermion_basis_general(N=L,Nf=(range(0,L,2),range(0,L,2)),kblock=0 )
