...
for state, state_sp in zip([V[:,0], V], [sp.csr_matrix(V[:,0]).T, sp.csr_matrix(V)]):
	...
	np.testing.assert_allclose(sent['Sent_A']-sent_sp['Sent_A'],0.0,atol=1E-5,err_msg='Failed Sent comparison!')
	np.testing.assert_allclose([sent['rdm_A'][i] - rdm_A for i,rdm_A in enumerate(sent_sp['rdm_A'])],0.0,atol=1E-5,err_msg='Failed rdm_A comparison!')
	...
	np.testing.assert_allclose([sent['rdm_B'][i] - rdm_B for i,rdm_B in enumerate(sent_sp['rdm_B'])],0.0,atol=1E-5,err_msg='Failed rdm_A comparison!')
