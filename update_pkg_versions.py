...
pkg_urls = {
    ...
    "boost": (
        "https://www.boost.org/doc/libs/1_70_0/libs/python/doc/html/index.html",
        "installation must include header files for boost.",
    ),
...
meta = meta_template.format(
    version_text=version_text, run_recipe=run_recipe, host_recipe=host_recipe
)
meta_omp = meta_omp_template.format(
    version_text=version_text, run_recipe=run_recipe, host_recipe=host_recipe
)
conda_build_config = conda_build_config_template.format(
    python_text=python_text, numpy_text=numpy_text
)
...
for pkg, url in pkg_urls.items():
    if len(url) > 1:
        md_text += [
            " * " + md_url.format(text=pkg, url=url[0]) + pkg_vers[pkg] + ", " + url[1]
        ]
        rst_text += [
            "    * "
            + rst_url.format(text=pkg, url=url[0])
            + pkg_vers[pkg]
            + ", "
            + url[1]
        ]
...
Installation_rst_lines = (
    Installation_rst_lines[: begin + 1] + rst_text + Installation_rst_lines[end:]
)
