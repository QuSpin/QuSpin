...
def ent_entropy(
    ...
    **_basis_kwargs,
    ...
    translate_dict.update(
        {
            "DM_chain_subsys": "rdm_A",
            "DM_other_subsys": "rdm_B",
            "both": "both",
            "lmbda": "p_A",
        }
    )
...
def diag_ensemble(
	...
	**Note: All expectation values depend statistically on the symmetry block used via the available number of 
    ...
    if _np.any(_np.diff(_np.sort(E2)) < 1e3 * _np.finfo(E2.dtype).eps):
        raise TypeError(
            "Cannot use function 'diag_ensemble' with dengenerate e'values 'E2'!"
        )
    del E2
    ...
    if delta_t_Obs or delta_q_Obs:
        if not Obs:
            raise TypeError(
                "Expecting to parse the observable 'Obs' whenever 'delta_t_Obs = True' or 'delta_q_Obs = True'!"
            )

    # calculate diagonal ensemble DM

    if isinstance(
        system_state, (list, tuple, _np.ndarray)
    ):  # initial state either pure or DM
        ...
        if "V1" in system_state.keys():
            ...
            raise TypeError(
                "Dictionary 'system_state' must contain states matrix 'V1'!"
            )
        ...
        if "E1" in system_state.keys():
            ...
            raise TypeError(
                "Dictionary 'system_state' must contain eigenvalues vector 'E1'!"
            )
        ...
        if "f_args" in system_state.keys():
            ...
            raise TypeError(
                "Dictionary 'system_state' must contain function arguments list 'f_args'!"
            )
        ...
        if "V1_state" in locals():
            ...
            if min(V1_state) < 0 or max(V1_state) > len(E1) - 1:
                raise TypeError(
                    "Value 'V1_state' violates '0 <= V1_state <= len(E1)-1'!"
                )
    ...
    if Srdm_Renyi:
        ...
        N_A = len(sub_sys_A)
        rdm_A = basis.partial_trace(
            V2, sub_sys_A=sub_sys_A, enforce_pure=True, **partial_tr_args
        )
...
def obs_vs_time(
    ...
    for key, val in Obs_dict.items():
        if not ishamiltonian(val):
            if not (_sp.issparse(val)) and not (
                val.__class__ in [_np.ndarray, _np.matrix]
            ):
    ...
    if len(Sent_args) > 0:
        ...
        if (
            ("chain_subsys" in Sent_args)
            or ("DM" in Sent_args)
            or ("svd_return_vec" in Sent_args)
        ):
    ...
    if return_state:
        for key, Obs in Obs_dict.items():
            Expt_time[key] = Obs.expt_value(
                psi_t, time=times, check=False, enforce_pure=enforce_pure
            )
        ...
        for m, psi in enumerate(psi_t):
            ...
            if verbose:
                print("obs_vs_time integrated to t={:.4f}".format(time))
...
def _ent_entropy(
    ...
    v, rho_d, N_A = _reshape_as_subsys(
        system_state, basis, chain_subsys=chain_subsys, subsys_ordering=subsys_ordering
    )
    del system_state
    ...
    if DM == False:
        if rho_d is not None and rho_d.shape != (
            1,
        ):  # need DM for Sent of a mixed system_state
            U, lmbda, _ = _npla.svd(v, full_matrices=False)
            DM_chain_subsys = _np.einsum(
                "n,nij,nj,nkj->ik", rho_d, U, lmbda**2, U.conj()
            )
            DM = "chain_subsys"
        else:
        ...
        if rho_d is not None:
            DM_other_subsys = _np.einsum(
                "n,nji,nj,njk->ik", rho_d, V.conj(), lmbda**2, V
            )
        else:
...
def _reshape_as_subsys(system_state, basis, chain_subsys=None, subsys_ordering=True):
    ...
    if chain_subsys is not None:
        ...
        if len(chain_subsys) == 0:
            ...
            raise TypeError(
                "'subsys' contains sites exceeding the total lattice site number!"
            )
            ...
            raise ValueError(
                "'subsys' must iterable of integers with values in {0,...,L-1}!"
            )
        elif subsys_ordering:
            if (
                len(set(chain_subsys)) == len(chain_subsys)
                and sorted(chain_subsys) != chain_subsys
            ):
                # if chain subsys is def with unordered sites, order them
                warnings.warn(
                    "'subsys' {} contains non-ordered sites. 'subsys' re-ordered! To change default set 'subsys_ordering = False'.".format(
                        chain_subsys
                    ),
                    stacklevel=4,
                )
    ...
    if isinstance(system_state, dict):
        ...
        if keys == set(["V_rho", "rho_d"]):
            ...
            if rho_d.shape != (basis.Ns,):
                raise ValueError(
                    "expecting a 1d array 'rho_d' of size {}!".format(basis.Ns)
                )
            ...
            if psi.shape != (basis.Ns, basis.Ns):
                raise ValueError(
                    "expecting a 2d array 'V_rho' of size ({},{})!".format(
                        basis.Ns, basis.Ns
                    )
                )
        ...
        if _sp.issparse(system_state):
            warnings.warn(
                "ent_entropy function only handles numpy.ndarrays, sparse matrix will be comverted to dense matrix.",
                UserWarning,
                stacklevel=4,
            )
        ...
        if psi.shape[0] != basis.Ns:
            raise ValueError(
                "V_states shape {0} not compatible with basis size: {1}.".format(
                    psi.shape, basis.Ns
                )
            )
    else:
        ...
        if system_state.ndim == 1:  # pure state
            ...
            if (
                _np.min(rho_d) < 0
                and abs(_np.min(rho_d)) > 1e3 * _np.finfo(rho_d.dtype).eps
            ):
    ...
    if basis.__class__.__name__[:-9] in ["spin", "boson", "fermion"]:
        ...
        if chain_subsys is None:
            ...
            warnings.warn(
                "Subsystem contains sites {}.".format(chain_subsys), stacklevel=4
            )
        ...
        if basis.Nph is not None:  # no total particle conservation
            Nph = basis.Nph
            if basis.Ns < photon_Hspace_dim(
                N, basis.Ntot, basis.Nph
            ):  # chain symmetries present
        ...
        raise ValueError(
            "'basis' class {} not supported!".format(basis.__class__.__name__)
        )
...
def _inf_time_obs(
    ...
    if delta_t_Obs is not False:
        ...
        # calculate diag ens value of Obs fluctuations
        if delta_q_Obs is not False:
            delta_q_Obs_d = _np.sqrt(
                _np.einsum("j...,j->...", rho, delta_q_Obs).real
                - delta_t_Obs_d
                - Obs_d**2
            )
