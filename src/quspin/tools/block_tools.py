...
def block_diag_hamiltonian(
    ...
    if all([isinstance(block, dict) for block in blocks]):
        ...
        raise ValueError(
            "blocks must be list of dictionaries containing symmetry sectors."
        )
...
def _block_evolve_helper(
    H, psi, t0, times, stack_state, imag_time, solver_name, solver_args
):
    ...
    def expm(
        ...
        **Arguments NOT described below can be found in the documentation for the `exp_op` class.**
        ...
        if H_list:
            P = _sp.hstack(P, format="csr")
            if iterate:
                return _block_expm_iter(
                    psi_blocks, H_list, P, start, stop, num, endpoint, n_jobs
                )
            else:
                ver = [int(v) for v in _scipy.__version__.split(".")]
                if (
                    H_is_complex
                    and (start, stop, num, endpoint) != (None, None, None, None)
                    and ver[1] < 19
                ):
                    mats = _block_expm_iter(
                        psi_blocks, H_list, P, start, stop, num, endpoint, n_jobs
                    )
                    return _np.array([mat for mat in mats]).T
                else:
                    psi_t = _Parallel(n_jobs=n_jobs)(
                        _delayed(_expm_multiply)(
                            H, psi, start=start, stop=stop, num=num, endpoint=endpoint
                        )
                        for psi, H in _izip(psi_blocks, H_list)
                    )
